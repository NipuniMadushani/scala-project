...
object AssertionModel:
  ...
  trait ResponsePredicate:
    ...
    /** Composes two predicates with a logical and.
      */
    def &&(other: ResponsePredicate): ResponsePredicate =
      AndPredicate(this, other)

    /** Composes two predicates with a logical or.
      */
    def ||(other: ResponsePredicate): ResponsePredicate =
      OrPredicate(this, other)

    private case class AndPredicate(a: ResponsePredicate, b: ResponsePredicate)
        extends ResponsePredicate:
      val fun = (r: Response) => a.fun(r) && b.fun(r)
      val description = s"${a.description} and ${b.description}"
    end AndPredicate

    private case class OrPredicate(a: ResponsePredicate, b: ResponsePredicate)
        extends ResponsePredicate:
      val fun = (r: Response) => a.fun(r) || b.fun(r)
        ...
        Success(tryResult match
          case Success(response) =>
            if (responsePredicate.fun(response))
              ...
              AssertionFailure(
                s"${description}: response did not match ${responsePredicate.description}"
              )
  ...
  case class ResponseContainsJson(referenceJsonObject: JsonObject)
    ...
    val description = s"jsonObject is ${JsonObject}"
